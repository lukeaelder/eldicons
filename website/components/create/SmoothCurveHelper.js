...
const SmoothCurveHelper = ({ keyVal, x, y, x2, y2, val1, val2, hoverControls }) => {
  ...
  return (
    <path
      className={`stroke-slate-500/60 stroke-[.2] transition duration-75 dark:stroke-slate-500/70  ${
        ...
        state.create.hovering.has(`SCH${keyVal}-${val1}-${val2}`)
          ? '!block !stroke-red-500 dark:!stroke-red-500'
          : ''
      }`}
      id={`SCH${keyVal}-${val1}-${val2}`}
      d={`M${x} ${y}L${x2} ${y2}`}
      strokeDasharray='0.5'
      onMouseEnter={() => {
        setState((p) => ({
...
export default SmoothCurveHelper;
